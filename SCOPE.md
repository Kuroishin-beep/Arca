...
| --- | --- | --- |
| **Framework** | **Next.js 15 (App Router), fullstack** | One deployable for UI and API. Server Components render the inventory table on the server, so the panel paints without shipping the whole dataset as JSON. Server Actions give mutations without hand-writing an API client for every verb. |
...
| **Styling** | **Tailwind CSS** over CSS custom properties | See [Design.md](final-project-planning/Design.md) Step A. Tokens in `:root`, theme in `tailwind.config.ts`. No gradients anywhere. |
| **Components** | Hand-built on the Design.md component table; **Radix UI primitives** for dialog, popover, dropdown | The mockups are already component-shaped. Radix supplies focus trapping, `Esc` handling and ARIA wiring for the modals — the parts that are tedious to get right and embarrassing to get wrong. |
...
[Sign in] --"authenticated"--> [Workspace]
        ...
        +--------------------------+--------------------------+
        |                          |                          |
 "select container"        "click Move on a row"       "click + Add item"
        |                          |                          |
        v                          v                          v
 [Workspace, container]      [Move item dialog]         [Item editor]
        |                          |                          |
 "click a row"              "confirm" -> back          "save" -> back
        v
 [Detail panel]  --"stretch: open character"--> [Character sheet]
